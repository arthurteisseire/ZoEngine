...
VkShaderModule Shader::loadShader(VkDevice device, std::string shaderFileName)
{
    ...
    {
        Logger::error("%s: could not load shader '%s'\n", __FUNCTION__, shaderFileName.c_str());
...
std::string Shader::loadFileToString(std::string fileName)
{
    ...
    {
        Logger::error("%s error: could not open file %s\n", __FUNCTION__, fileName.c_str());
        Logger::error("%s error: system says '%s'\n", __FUNCTION__, strerror(errno));
        return std::string();
    }

    if (inFile.bad() || inFile.fail())
    {
        Logger::error("%s error: error while reading file %s\n", __FUNCTION__, fileName.c_str());
        inFile.close();
        return std::string();
    }

    inFile.close();
    Logger::error("%s: file %s successfully read to string\n", __FUNCTION__, fileName.c_str());
    return str;
}
