...
bool Texture::loadTexture(VkRenderData &renderData, const std::string &textureFilename)
{
    int texWidth;
    int texHeight;
    int numberOfChannels;

    unsigned char *textureData = stbi_load(textureFilename.c_str(), &texWidth, &texHeight, &numberOfChannels,
                                           STBI_rgb_alpha);

    if (!textureData)
    {
        Logger::error("%s error: could not load file '%s'\n", __FUNCTION__, textureFilename.c_str());
    ...
    {
        Logger::error("%s error: could not allocate texture image via VMA\n", __FUNCTION__);
        return false;
    }
    ...
    {
        Logger::error("%s error: could not allocate texture staging buffer via VMA\n", __FUNCTION__);
        return false;
    }
    ...
    {
        Logger::error("%s error: failed to reset staging command buffer\n", __FUNCTION__);
        return false;
    }
    ...
    {
        Logger::error("%s error: failed to begin staging command buffer\n", __FUNCTION__);
        return false;
    }
    ...
    if (vkCreateImageView(renderData.rdVkbDevice.device, &texViewInfo, nullptr, &renderData.rdTextureImageView) !=
        VK_SUCCESS)
    {
        Logger::error("%s error: could not create image view for texture\n", __FUNCTION__);
        return false;
    }
    ...
    if (vkCreateSampler(renderData.rdVkbDevice.device, &texSamplerInfo, nullptr, &renderData.rdTextureSampler) !=
        VK_SUCCESS)
    {
        Logger::error("%s error: could not create sampler for texture\n", __FUNCTION__);
        return false;
    }
    ...
    if (vkCreateDescriptorPool(renderData.rdVkbDevice.device, &descriptorPool, nullptr, &renderData.rdDescriptorPool) !=
        VK_SUCCESS)
    {
        Logger::error("%s error: could not create descriptor pool\n", __FUNCTION__);
        return false;
    }
    ...
    if (vkAllocateDescriptorSets(renderData.rdVkbDevice.device, &descriptorAllocateInfo, &renderData.rdDescriptorSet) !=
        VK_SUCCESS)
    {
        Logger::error("%s error: could not allocate descriptor set\n", __FUNCTION__);
        return false;
    }
    ...
    Logger::info("%s: texture '%s' loaded (%dx%d, %d channels)\n", __FUNCTION__, textureFilename.c_str(), texWidth,
                 texHeight, numberOfChannels);
    return true;
}
